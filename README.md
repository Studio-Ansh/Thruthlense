...
- Domain monitoring: flags and tracks newly identified low-credibility/high-risk sources, with auto-generated weekly misinformation trend reports.
...
- Explicit, disclosed fallback: if the live Gemini forensics call fails, the API returns a clearly-labeled local heuristic result rather than silently faking a "live" score.
...
| **Production Build** | esbuild (bundles `server.ts` → `dist/server.cjs`), Vite (static assets) |
...
flowchart TD
    ...
    FE -->|multipart/form-data or JSON| API["Express API Layer<br/>(server.ts)"]
...
**Offline ML training track** (independent of the live API path above): `image_model/` and `video_model/` contain standalone PyTorch training pipelines (`train.py`, `evaluate.py`, `verify_image.py`, `verify_video.py`) for the EfficientNet-B4 / ViT and ResNeXt50 + Bi-LSTM backbones. These are trained/evaluated against Kaggle datasets (FaceForensics++, DFDC, and similar) and are designed to be swapped in as the production inference backbone in place of, or alongside, the live Gemini calls.
...
│   ├── components/             # Dashboard UI (Telemetry Playground, Pipeline Graph, Login, etc.)
│   ├── lib/                    # Firebase client + API helpers
│   ├── App.tsx
│   └── main.tsx
├── server.ts                   # Express API — routes, Gemini orchestration, fusion logic
├── api/index.ts                # Vercel serverless entry point (wraps server.ts)
...
├── utils/metadata_extractor.py  # EXIF / provenance metadata parsing
...
Ensure `firebase-applet-config.json` exists in the project root with your Firebase project's web config:
```json
{
...
npm run dev
```
This starts the Express backend (Port `3000`) with Vite-powered hot-reloaded frontend assets.

### 7. Build & run for production
```bash
npm run build   # Bundles server.ts -> dist/server.cjs and builds frontend static assets
...
python video_model/model/train.py

# Evaluate
python image_model/evaluate.py

# Run standalone CLI verification
python video_model/verify_video.py --video path/to/file.mp4 --report forensic_report.json
```
